package no.novari.fint.consumer.models.tilgang;
...
public class TilgangController {
    ...
    public TilgangResources getTilgang(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getTilgangByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Tilgang cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, NoarkActions.GET_ALL_TILGANG, client);
    ...
    @PostMapping("/$query")
    public TilgangResources getTilgangByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getTilgang(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private TilgangResources getTilgangByOdataFilter(
        ...
        Event event = new Event(
                ...
                NoarkActions.GET_TILGANG, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new TilgangResources();
    
        ArrayList<TilgangResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<TilgangResource>>() {});
    ...
    public TilgangResource getTilgangBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, NoarkActions.GET_TILGANG, client);
        event.setOperation(Operation.READ);
